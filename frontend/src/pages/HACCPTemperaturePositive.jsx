...
export default function HACCPTemperaturePositive() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [schedeRes, chiusureRes] = await Promise.all([
        api.get(`/api/haccp/temperature-positive/schede/${anno}`),
        api.get(`/api/haccp/chiusure/anno/${anno}`)
      ]);
      
      const schedeMap = {};
      schedeRes.data.forEach(s => { schedeMap[s.frigorifero_numero] = s; });
      setSchede(schedeMap);
  ...
  const popolaFrigoriferi = async () => {
    try {
      await api.post(`/api/haccp/temperature-positive/popola/${anno}`);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, display: 'flex', alignItems: 'center', gap: 8 }}>
            🌡️ Temperature Frigoriferi (Positive)
          </h1>
