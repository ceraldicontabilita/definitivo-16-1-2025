...
import { Thermometer, RefreshCw, ChevronLeft, ChevronRight, Printer, FileText } from "lucide-react";
...
const TemperaturePositiveView = () => {
  ...
  const fetchSchede = useCallback(async () => {
    setLoading(true);
    try {
      // Carica tutte le 12 schede frigoriferi + chiusure
      const promises = [];
      for (let i = 1; i <= 12; i++) {
        promises.push(axios.get(`${API}/temperature-positive/scheda/${anno}/${i}`));
      }
  ...
  // Ottieni temperatura per un frigorifero in un giorno
  ...
  const getCellDisplay = (frigoNum, giorno) => {
    ...
    if (typeof record === 'object') {
      ...
      if (record.temp !== undefined && record.temp !== null) {
        ...
        return {
          value: `${temp}°`,
          class: fuoriRange ? "bg-red-100 text-red-700 font-bold" : "bg-orange-50 text-orange-800",
  ...
  const stampaScheda = () => {
    ...
    for (let g = 1; g <= numGiorni; g++) {
      ...
      for (let f = 1; f <= 12; f++) {
        ...
        righe += `<td style="padding:4px; border:1px solid #ccc; text-align:center; ${
          ...
          cell.class.includes('orange') ? 'background:#fff7ed;' : ''
        }">${cell.value}</td>`;
      }
      righe += '</tr>';
    }
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Temperature Frigoriferi - ${MESI_IT[mese-1]} ${anno}</title>
        <style>
      ...
      <body>
        <div class="header">
          <h1>🌡️ SCHEDA TEMPERATURE FRIGORIFERI</h1>
        ...
        <table>
          <thead>
            <tr>
              <th>G</th>
              ${Array.from({length: 12}, (_, i) => `<th>F${i+1}</th>`).join('')}
            </tr>
          </thead>
          <tbody>${righe}</tbody>
        </table>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Tabella - Giorni come righe, Frigoriferi come colonne */}
      ...
      <div className="flex items-center gap-4 text-xs text-gray-600 bg-gray-50 p-3 rounded-lg flex-wrap">
        <span className="flex items-center gap-1">
          <span className="w-4 h-4 bg-orange-50 border rounded"></span> Temp OK
        </span>
        <span className="flex items-center gap-1">
